...
A similar analysis can be performed for the L\"uscher and Weisz action, taking into account the contributions from the rectangle terms \textcolor{red}{[appendix proof]}. The L\"uscher and Weisz action written in the same form as Eq.~\ref{eq:WilsonMomentum} is
%
\begin{equation}
...
\end{equation}
%
In this work we therefore make the variable substitution $p_\mu\rightarrow q_\mu$ to ensure that at high momentum where we expect the gluon propagator to tend towards tree level we observe $q^2 D(q^2) = 1$. The momentum variables chosen for both the Wilson and L\"uscher and Weisz action have been numerically verified to provide better tree level agreement in Refs.~\cite{Marenzoni:1994ap, Bonnet:2001uh}.\\
